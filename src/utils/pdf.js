...
const { NEW_CAMPAIGN_STATUS, SEX } = require('../config')
const { getFormatedNumber, parseDate } = require('./transformData')
...
const getDocDefinition = ({ brand, name, orderNumber, startDate, endDate, status, summary, publishers, target, user, amount, locations = [], ages = [], sector, sex, url }) => {
          ...
          text: [
            ...
            },
            {
              text: [{
                ...
                text: `${ages?.map(({ label }) => label).join(', ')} \n`
              }]
            },
            {
              text: [{
                ...
                text: `${locations?.map(({ label }) => label).join(' - ')} \n`
              }]
            }
          ]
        }
...

    {
      ...
      table: {
        ...
        body: [
          ...
          ['', '', '', '', { text: 'Sub total:', bold: true, alignment: 'right', fontSize: 10 }, { text: `$${getFormatedNumber(amount)}`, alignment: 'right', fontSize: 10 }],
          ['', '', '', '', { text: 'Iva:', bold: true, alignment: 'right', fontSize: 10 }, { text: `$${getFormatedNumber(iva)}`, alignment: 'right', fontSize: 10 }],
          ['', '', '', '', { text: 'Total:', bold: true, alignment: 'right', fontSize: 10 }, { text: `$${getFormatedNumber(total)}`, alignment: 'right', fontSize: 10 }]
        ]
      },
